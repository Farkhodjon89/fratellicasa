...
const ApplicationInfo = ({ cartItems }) => {
  ...
  const [country, setCountry] = useState(
    user && user.country ? user.country : 'Uzbekistan'
  )
  const [city, setCity] = useState(
    user && user.city ? user.city : 'Toshkent shahri'
  )
  ...
  const [feedback, setFeedback] = useState(
    user && user.feedback ? user.feedback : ''
  )
          ...
          <div className={s.form}>
            ...
            <div className={s.deliveryInfo}>
              ...
              <div className={s.extraData}>
                При покупке на сумму свыше 300,000 действует бесплатная
                доставка.
              </div>
            </div>
