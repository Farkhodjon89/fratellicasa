...
const ProductCard = ({
  ...
  const [selectedProductId, setselectedProductId] = useState(
      product.variations ? product.variations.nodes[0].databaseId : product.databaseId,
  );
  const quantityCount = 1;
  // product card images
  const [selectedProductImage, setSelectedProductImage] = useState(
      product.variations ? product.variations.nodes[0]?.image?.sourceUrl : product.image?.sourceUrl,
  );
  ...
  product.productCategory.nodes.map(({name}) => {
    ...
    productCategory = !['Uncategorized', 'Женская', 'Мужская', 'Девочкам'].includes(name)
        ? name
        : '';
  });
  ...
  const addToCartOpenCart = () => {
    ...
    fbq('track', 'AddToCart', {
      value: product.onSale ? product.woocsSalePrice : product.woocsRegularPrice,
    });
  };
  ...
  let sizes = product.variations
      ? product.variations.nodes.map(({attributes, stockQuantity, databaseId}) => ({
  ...
  const ORDER = ['one size', 'xxs', 'xs', 's', 'm', 'l', 'xl', '2xl','3xl','4xl', 'xxl']
          ...
          <div className={s.details}>
            ...
            <div className={s.price}>
              {product.onSale ? (
                  ...
                  <span className={s.salePrice}>{getPrice(getPriceValue(product))}</span>
              )}
            </div>
                      ...
                      <div className={s.color} key={uuidv4()}>
                        ...
                        <div>
                          ...
                          topColors.map(({slug, paColors}) => (
                              <div key={uuidv4()} className={product.slug == slug && s.active}>
                      ...
                      <div className={s.size} key={uuidv4()}>
                        ...
                        <div>
                          {sizes.map(({size, stockQuantity, databaseId}, index) => (
                              <button
                                  key={uuidv4()}
                                  className={` ${selectedProductSize === size ? s.active : ''} ${
                  ...
                  {product.attributes && product.attributes.nodes[0] ? (
                      <div className={s.color}>
                        ...
                        <div>
                          <div
                              className={
                                selectedProductColor === product.attributes.nodes[0].options[0]
                                    ? s.active
                                    : ''
                              }>
                            <button
                                ...
                                onClick={() => {
                                  ...
                                  setSelectedProductColorCode(product.terms.nodes[0].color);
                                  setSelectedProductColor(product.attributes.nodes[0].options[0]);
                                  setSelectedProductSize(product.attributes.nodes[1].options[0]);
                                }}
                            />
                          </div>
                        </div>
                      ...
                      <div className={s.size}>
                        ...
                        <div>
                          <button
                              key={uuidv4()}
                              className={
                                selectedProductSize === product.attributes.nodes[1].options[0]
                                    ? s.active
                                    : ''
                              }
                              onClick={() => {
            ...
            {product.variations && (
                <SizeGrid activeStatus={open} getActiveStatus={setOpen} product={product}/>
            )}
            <div className={s.actions}>
              <button
                  className={`${s.actionAddToCart} ${cartItem || !selectedProductSize ? s.active : ''}`}
                  onClick={
                    cartItem ? () => deleteFromCart(selectedProductId) : () => addToCartOpenCart()
                  }
              ...
              <button
                  className={`${s.actionAddToWishlist} ${wishlistItem !== undefined ? s.active : ''}`}
                  onClick={
